...
var Loklak = module.exports = function () {
	this.httpClient = request.createClient('http://api.loklak.org/api/');
};
...
Loklak.prototype.user = function (params, cb) {
	...
	var url = 'user.json?screen_name=' + params.name;
	if (params.followers !== undefined) {
		url += '&followers=' + params.followers;
	}
	if (params.following !== undefined) {
		url += '&following=' + params.following;
	}
...
Loklak.prototype.aggregations = function (params, cb) {
	apiUrl = "search.json?";
	if (params.q === undefined) {
		cb({});
		return;
	...
	if (params.fields !== undefined) {
		apiUrl += '&fields=' + params.fields.join(',');
	}
